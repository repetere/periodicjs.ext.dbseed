...
var setupCompleteSeedFile = function (options, cb) {
	...
	return function () {
		...
		else { completeFile(writePath || './seed_data.json', 'utf8', cb); }
	};
};
	...
	try {
		return Object.keys(model_settings).reduce((streams, key) => {
			...
			streams[key].on('data', data => {
				data = data.toJSON();
				transformDataForSeed(data, transform_configuration[key])
					.then(transformed => {
						let writeData = {
							datatype: key,
...
var createSeed = function (options, cb) {
	let fn = function (callback) {
		...
		try {
			transform_configuration = require(path.join(__dirname, '../../../config/extensions/periodicjs.ext.dbseed/customseed.js')).exportseed;
		}
		catch (e) {
