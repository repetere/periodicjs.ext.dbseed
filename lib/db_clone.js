...
var configureWriteQueue = function (writePath, usePartition, batchSize, onDrain) {
	console.log('this is the batchSize', batchSize);
	...
	let createWriteStreamSwitchboard = function (data) {
		if (streamHolder._current.path !== writePath.replace(/(.+)(\.json)/, `$1_${data.datatype}${ (typeof batchSize === 'number') ? '('+ (batches[data.datatype] || 0) +')' : '' }$2`)) {
			if (!streamHolder[data.datatype] || !streamHolder[data.datatype].fd) {
				let wp = writePath.replace(/(.+)(\.json)/, `$1_${data.datatype}${ (typeof batchSize === 'number') ? '('+ (batches[data.datatype] || 0) +')' : '' }$2`);
	...
	let writeStream = (usePartition === true) ? createWriteStreamSwitchboard : createDataTransformStream();
	...
	let queue = async.queue(function (d, cb) {
		process.nextTick(() => {
			...
			if (typeof writeStream === 'function' && typeof batchSize !== 'number') { isLast = (records[d.data.datatype] >= d.count); }
				...
				if ((typeof batch_records === 'number' && batch_records >= batchSize) || (batch_records[d.data.datatype] >= batchSize)) {
					isLast = true;
					if (!usePartition) {
						batch_records = 0;
						batches++;
			...
			writeToFile({
				writeStream: (typeof writeStream === 'function') ? writeStream(d.data) : writeStream,
				writeData: JSON.stringify(d.data),
				isLast
			});
			if (isLast && typeof batchSize === 'number') {
				process.nextTick(() => {
					if (usePartition) { streamHolder[d.data.datatype] = {}; }
					else {
...
var configureQueryStreams = function (options) {
	...
	try {
		...
		return async.parallelAsync(Object.keys(model_settings).reduce((results, key) => {
			if (skip_models.indexOf(key) === -1) {
				...
				results[key] = function (cb) {
					getMongoQueryStream(mongoOptions)
